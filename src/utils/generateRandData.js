...
function generatePhoneNumber() {
  return '080' + getRandomNumber(10000000, 99999999).toString();
}
...
const descriptionOfTheAbuse = ["Psychological", "Economic", "Sexual", "Physical"]
const relationshipToThePerpetrator = ["Spouse", "Parent", "Boss", "Sibling", "Stranger", "Child", "Relative", "Friend", "Neighbor", "Co-worker"]
const frequencyOfTheAbuse = ["Weekly", "Daily", "Monthly", "Once", "Yearly"]
const occupation = ["Teacher", "Nurse", "Engineer", "Student", "Trader", "Farmer", "Business", "Driver", "Housewife", "Civil Servant", "Artisan", "Unemployed", "Retired"]
const tribes = [
  'Afo',
  'Anang',
  'Bachama',
...
function getRandomStateAndLGA() {
  const states = Object.keys(lgasByState);
  const state = getRandomItem(states);
  const lgaObj = getRandomItem(lgasByState[state]);
  return { state, lga: lgaObj.name, coordinates: { latitude: lgaObj.lat, longitude: lgaObj.lng } };
}

function generateVictimData(n) {
  const data = [];
  for (let i = 0; i < n; i++) {
    const { state, lga, coordinates } = getRandomStateAndLGA();

    data.push({
      "Age": getRandomNumber(18, 60),
      "Description of the Abuse": getRandomItem(descriptionOfTheAbuse),
      "Relationship to the Perpetrator": getRandomItem(relationshipToThePerpetrator),
      "Frequency of the Abuse": getRandomItem(frequencyOfTheAbuse),
      "Occupation": getRandomItem(occupation),
      "Phone Number": generatePhoneNumber(),
      "Number of Children": getRandomNumber(0, 5),
      "Gender": getRandomItem(["Male", "Female"]),
      "State": state,
      "LGA": lga,
      "Tribe": getRandomItem(tribes),
      "Socio-economic Status": getRandomItem(socioEconomicStatuses),
      "Marital Status": getRandomItem(maritalStatuses),
...
console.log(JSON.stringify(generateVictimData(50), null, 2));
